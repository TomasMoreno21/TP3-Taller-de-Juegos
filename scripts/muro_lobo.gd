...
func _physics_process(_delta: float) -> void:
	...
	if dist_x > assist_distancia or dist_y < -55.0 or dist_y > 55.0:
		return
	var p_body := player as CharacterBody2D
	if p_body.test_move(Transform2D(0, Vector2(0, -assist_altura)), Vector2.ZERO):
		return
	if p_body.test_move(Transform2D(0, Vector2(0, -assist_altura)).translated(Vector2(0, 0)), Vector2(p_body.get("facing") * 8, 0)):
		return
