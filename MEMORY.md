...
- **Sesión 25/09 (fondo caverna: más densidad + profundidad, 3ra pasada):** el usuario pidió "que se sienta más lleno" y luego "mucha densidad de estalactitas, estalagmitas, piedras y cosas, que muestre profundidad". Solución: **dobles hileras por capa (trasera tenue + principal)** — estalactitas con `alfa 0.28-0.42` atrás (ancho/420) y familia densa adelante (ancho/520, 3-5 por familia); estalagmitas igual (trasera ancho/480 tenue + principal ancho/480); **hebras MUY largas (`ancho/600`, h 1200-1900)** que bajan hasta mitad del abismo; **nueva función `_dibujar_roca(x, y, s, h, alfa, claro)`** (media-elipse convexa segura con luz de borde) usada en: pared (rejilla densa `ancho/380 × alto/340`, tamaños 14-60, alphas 0.22-0.5 para lejanía + fallas), sombra (guijarros tenues `ancho/700`, alphas 0.1-0.24), estalactitas (rocas en la masa del techo `ancho/620`), estalagmitas (guijarros + "pilares rotos" `ancho/900` con `_dibujar_roca` alto + falla) y pilares (guijarros en la base, columnas cada `ancho/700`). Todo convexo sin jitter en la media-elipse → sin "Invalid polygon". Verificado: import limpio, `diag_cueva` 6/6 OK, autotest `FALLOS = 0`. Ajuste rápido de densidad: cambiar los divisores de cada rejilla (menor = más elementos).
