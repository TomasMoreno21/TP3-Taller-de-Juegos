...
- Pendiente de migrar a edición desde editor: **las olas** de `encounter.gd` (datos → recursos/escenas). (Formas y enemigos YA no usan `.tres` por decisión del usuario — ver arriba.)
...
- **(13/08) El buffer con "solo si mantenés el botón":** el usuario prefirió que el ataque guardado durante el cooldown **solo se ejecute si seguís apretando** cuando termina (`_sigue_apretado(tipo)` en `_lanzar_buffered`); si soltaste, se cancela. Los tests presionan tras la recuperación, así que siguen pasando.
